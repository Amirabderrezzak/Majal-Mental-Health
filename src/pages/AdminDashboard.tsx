...
import {
  ...
  TrendingUp, Clock, UserCheck, AlertCircle, ChevronDown,
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (tab === "dashboard") {
      ...
      apiFetch("/api/admin/reviews")
        ...
        .catch(() => toast.error("Erreur de chargement des avis"))
        .finally(() => setLoading(false));
    }
  }, [tab, apiFetch]);
  ...
  const toggleAdmin = async (userId: string, current: boolean) => {
    try {
      await apiFetch(`/api/admin/users/${userId}/grant-admin`, {
        method: "PATCH",
      ...
      toast.error("Échec de la mise à jour admin");
    }
  };
  ...
  const Sidebar = () => (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white flex flex-col shadow-2xl transition-transform duration-300 md:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="p-4 border-t border-gray-700">
        <button
          onClick={() => signOut?.()}
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-gray-400 hover:bg-gray-800 hover:text-red-400 transition-colors cursor-pointer border-none bg-transparent"
        >
    ...
    <div>
      ...
      {loading ? <Loader /> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          <StatCard label="Revenus (DZD)"        value={`${stats?.totalRevenue?.toLocaleString() ?? 0} DA`} icon={<Crown className="w-6 h-6 text-yellow-600" />} color="bg-yellow-50" />
        </div>
      )}
    </div>
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {(["all", "patient", "psychologue", "pending"] as const).map(f => (
            <button key={f} onClick={() => setUserFilter(f)} className={`px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer border transition-colors ${userFilter === f ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {["Nom", "Email", "Type", "Statut", "Ville", "Actions"].map(h => (
            ...
            <tbody>
              {filteredUsers.map(u => (
                <tr key={u.user_id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5"><span className="capitalize text-gray-600">{u.user_type === "psychologue" ? "Psy" : "Patient"}</span></td>
                  <td className="px-5 py-3.5">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusBadge[u.approval_status] || ""}`}>{u.approval_status}</span>
                  </td>
              ...
              {filteredUsers.length === 0 && (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400 text-sm">Aucun utilisateur trouvé.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {loading ? <Loader /> : (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                {["Patient", "Psychologue", "Date", "Durée", "Prix", "Statut"].map(h => (
                  <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {bookings.map(b => (
                <tr key={b.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusBadge[b.status] || "bg-gray-100 text-gray-600"}`}>{b.status}</span>
                  </td>
                </tr>
              ))}
              {bookings.length === 0 && (
    ...
    <div>
      ...
      {loading ? <Loader /> : (
        <div className="grid gap-4">
          ...
          {reviews.length === 0 && (
            <div className="text-center py-12 text-gray-400 text-sm">Aucun avis pour l'instant.</div>
          )}
        </div>
      )}
    </div>
  ...
  const AdminsTab = () => {
    ...
    return (
      <div>
        ...
        <p className="text-gray-500 text-sm mb-6">Accordez ou révoquez les droits d'administration à d'autres utilisateurs.</p>
        {loading ? <Loader /> : (
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              {nonAdmins.map(u => (
                <div key={u.user_id} className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50 last:border-none">
                  <div>
                    <p className="font-medium text-gray-900">{u.full_name || "—"}</p>
                    <p className="text-xs text-gray-500">{u.email} — <span className="capitalize">{u.user_type}</span></p>
                  </div>
