...
const Inscription = () => {
  ...
  const handleGoogle = async () => {
    setLoading(true);
    try {
      // NOTE: Supabase OAuth `queryParams` are forwarded to the provider's own
      // consent screen (Google) — they are never stored on the Supabase user or
      // passed to handle_new_user(), so there is no way to carry `userType`
      // through Google sign-up. This button is only ever rendered for the
      // patient path (see below) for exactly that reason: a psychologist
      // signing up this way would silently become an unlisted patient with
      // none of their professional fields (order number, specialty, city)
      // collected.
      ...
      <div className="min-h-[calc(100vh-72px)] flex items-center justify-center bg-teal-hero px-4 py-12">
        <div className="bg-card rounded-2xl shadow-card p-8 sm:p-10 w-full max-w-[480px]">
          ...
          {userType === "patient" ? (
            ...
            <p className="text-xs text-muted-foreground bg-teal-hero border border-border rounded-xl px-4 py-3 mb-6">
              {t("auth.googlePsyUnavailable")}
            </p>
          )}
