...
export default function PatientSidebar({
  ...
  return (
      ...
      <div className="px-6 py-5 border-b border-border/50 flex items-center justify-between">
        ...
        <button type="button" onClick={() => setSidebarOpen(false)} aria-label={t("common.closeMenu")} className="lg:hidden bg-transparent border-none cursor-pointer text-muted-foreground hover:text-foreground p-2.5 -m-2.5 focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:outline-none">
          <X className="w-5 h-5" />
        </button>
      </div>
            ...
            <li key={item.id}>
              ...
              >
                ...
                {item.id === "notifications" && unreadCount > 0 && (
                  <span className="w-5 h-5 bg-destructive text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-sm shrink-0">
                    {unreadCount}
                  </span>
                )}
              </button>
      ...
      <div className="px-5 py-4 border-t border-border/50 bg-teal-hero/30">
        ...
        <button
          onClick={signOut}
          className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-xs font-semibold text-destructive hover:bg-destructive/10 hover:text-destructive transition-all bg-transparent border-none cursor-pointer"
        >
